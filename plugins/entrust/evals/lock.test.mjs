...
function spawnRun(dir, { scenario = "stalled-turn", args = [], shim = survivorShim, env = {} } = {}) {
  const { child: p, done, stderrSoFar } = spawnNode(
    ...
    { env: { PATH: `${shim}:${process.env.PATH}`, FAKE_SCENARIO: scenario,
             ENTRUST_STATE_DIR: STATE_DIR, ...env } });
...
test("a second --prompt-file is a usage error, not a silently ignored one",
  ...
  async () => {
    ...
    const p = spawn(process.execPath, [DRIVER, "--prompt-file", a, "--prompt-file", b, "--prompt", "x"],
      { env: { ...process.env, PATH: `${shimDir}:${process.env.PATH}`, ENTRUST_STATE_DIR: STATE_DIR },
...
test("a cancelled config probe does not empty the shared home's config",
  ...
  async () => {
    ...
    const p = spawn(process.execPath,
      ...
      { env: { ...process.env, PATH: `${shimDir}:${process.env.PATH}`, FAKE_SCENARIO: "happy",
               FAKE_CONFIG_HANG: "1", ENTRUST_STATE_DIR: state },
        stdio: ["ignore", "ignore", "pipe"] });
