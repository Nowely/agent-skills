...
function plantAgent(w, dirname, { pid, identity, reportPath, line } = {}) {
  ...
  fs.writeFileSync(path.join(d, "err.txt"), line !== undefined ? line
    : `entrust: pid=${pid} identity=${identity} reportPath=${reportPath}\n`);
  return d;
}
...
function plantReported(w, { dataId = "entrust-other" } = {}) {
...
test("12 · a job record naming an item keeps it, and a record with no identity is alive",
  ...
  async () => {
    ...
    const c = liveChild();
    const dir = plantEval(w, "entrust-test-JJJJ");
...
test("13 · the eval scratch: an empty listing is an answer, a running suite is not, and a failed listing keeps every row",
  ...
  async () => {
    ...
    const dirs = [plantEval(w, "entrust-test-XXXX"), plantEval(w, "codex-lock-YYYY"),
    ...
    r = await list(w, { env: { ENTRUST_CLEANUP_PS: path.join(w.root, "no-such-listing") } });
    bad = need(w, r); if (bad) return bad;
    for (const d of dirs) {
      const row = rowAt(r.j, d);
...
test("16 · the snapshot binds every field it carries, one field at a time",
  ...
  async () => {
    ...
    const parts = [
      { what: "its size alone", field: "bytes", prefix: "entrust-test-SIZE",
        mutate: (w, dir) => {
...
test("18 · a symlink on the way in keeps the item; one inside a removed item is unlinked, its target untouched",
  ...
  async () => {
    ...
    // (c) links inside a removed item.
    {
      ...
      const dir = plantEval(w, "entrust-test-LINKS");
...
test("19 · what a yes covers, what a number covers, and what neither can reach",
  ...
  async () => {
    ...
    const theirs = plantRun(w, slugOf(path.join(w.root, "elsewhere")), "run-theirs",
      { A: report(path.join(w.root, "elsewhere")) });
    const scratch = plantEval(w, "entrust-test-SETS");
...
test("20 · the listing is three lines a person reads, and --json carries the same bytes",
  ...
  async () => {
    ...
    const narrow = await list(w, { env: { ENTRUST_CLEANUP_COLUMNS: "60" } });
...
test("24 · the snapshot's paths are compared, and a forged one never reaches a root",
  ...
  async () => {
    ...
    const dir = plantEval(w, "entrust-test-FORGE");
    plantDriverState(w);
    plantReported(w);
    const m = misses();
    const s = await snapshot(w);
...
test("26 · a file a read refuses and a pipe where a file belongs keep the item, and neither is ever deleted",
  ...
  async () => {
    ...
    const walledDir = plantEval(w, "entrust-test-WALL");
...
test("30 · the command the user is handed is safe to paste",
  ...
  async () => {
    ...
    const id = `entrust-a b'c"d;e$(touch pwned-sub)f\`touch pwned-tick\`g`;
    ...
    const r = await list(w);
    const bad = need(w, r); if (bad) return bad;
    const entry = (r.j.manual ?? []).find((e) => String(e.command ?? "").includes("entrust-a b"));
...
test("39 · identity is `dev:ino`, and a filesystem that recycles one is the whole of its limit",
  ...
  async () => {
    ...
    const dir = plantEval(w, "entrust-test-REUSE");
    const m = misses();
    const s = await snapshot(w);
    const bad = need(w, s); if (bad) return bad;
    const row = rowAt(s.j, dir);
...
test("40 · the data directory left by this plugin's previous name is offered by number; another copy's is not",
  ...
  async () => {
    const w = makeWorld("previous-name");
    const previous = path.join(w.data, "codex-delegate-nowely");
    fs.mkdirSync(path.join(previous, "jobs"), { recursive: true });
    fs.writeFileSync(path.join(previous, "jobs", "th-old.json"),
      JSON.stringify({ threadId: "th-old", pid: DEAD_PID, identity: DEAD_IDENTITY, cwd: "/nowhere",
                       started: "2026-09-01T00:00:00.000Z", endedAt: "2026-09-01T00:01:00.000Z", exitCode: 0 }));
    const other = plantReported(w, { dataId: "entrust-nowely" }).datadir;
    const m = misses();
    const s = await snapshot(w);
    const bad = need(w, s); if (bad) return bad;
    const old = rowAt(s.j, previous);
    m.ok(old, `the previous name's data directory is not listed at all: ${shown(s.j)}`);
    if (old) {
      m.re(old.name, /previous name, codex-delegate/, "the row's name");
      m.eq(old.selectable, true, "the previous name's data directory selectable");
      m.eq(old.proposed, true, "the previous name's data directory proposed");
      // Offered in one way only: a row a number removes must not also be a command telling the user
      // to remove it themselves.
      m.ok(!(s.j.manual ?? []).some((e) => String(e.command ?? "").includes(previous)),
        `it is offered by number and by hand at once: ${JSON.stringify(s.j.manual)}`);
    }
    const theirs = rowAt(s.j, other);
    m.ok(theirs, `the other copy's data directory is not listed at all: ${shown(s.j)}`);
    if (theirs) {
      m.eq(theirs.kind, "datadir", "the other copy's row kind");
      m.ok(!/previous name/.test(theirs.name),
        `the other copy is named as this plugin's own former data: ${theirs.name}`);
      m.eq(theirs.selectable, false, "the other copy's data directory selectable");
    }
    if (old) {
      const d = await pick(w, s.file, [old.n]);
      m.eq(d.code, 0, `its number exited ${d.code}: ${(d.err || d.out).trim().slice(0, 200)}`);
