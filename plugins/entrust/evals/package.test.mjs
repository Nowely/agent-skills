...
test("an entrust@ tag on HEAD is the version the tree claims",
  "the tag is what a user installs at; a tree that says 0.6.0 under an entrust@0.7.0 tag ships the wrong driver under the right name",
  () => {
    ...
    const tags = git(["tag", "--list", "entrust@*", "--points-at", "HEAD"]);
    ...
    if (!onHead.length) return skip("HEAD carries no entrust@ tag; the version is compared at the tag, not before it");
    const wrong = onHead.filter((t) => t.slice("entrust@".length) !== VERSION);
...
test("no local artifact is in the payload",
  "`source: \"./\"` makes every tracked file part of the install; a committed .claude/settings.local.json ships one machine's model pin to every user, and a stale .entrust.lock ships a lock",
  () => {
...
test("an entrust@ tag on HEAD that disagrees with the tree is a failure",
  ...
  async () => {
    ...
    if (g("tag", "entrust@9.9.9").status !== 0) return "git tag failed in the scratch repository";
    const { code, out } = await runSuiteIn(d);
    if (code === 0) return `an entrust@9.9.9 tag over ${VERSION} left the suite green: ${out.trim().split("\n").at(-1)}`;
    return /HEAD carries entrust@9\.9\.9 and the tree says/.test(out)
