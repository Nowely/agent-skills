...
grep -oE '"skill":"(entrust:)?codex"' <transcript>.jsonl | wc -l   # plugin input is entrust:codex
```
...
**while the turn is live**, not merely after it: the defect being pinned is a turn's `git add -A` staging
the lock, and by the time the run ends the lock has already been released.
...
(`ENTRUST_SCHEMA_DIR` is the upgrade override), so a regeneration that changes a shape goes red
there.
...
`grep -oE '"skill":"(codex-delegate:)?codex-delegate"' <transcript>.jsonl | wc -l`, the skill's id on that day): case 21
...
skipped eleven scenarios), and `ENTRUST_LIVE_TURN=1` diffs live item key sets against fixture
