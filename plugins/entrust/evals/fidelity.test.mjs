...
async function liveTurns() {
  ...
  // --- one read-level turn: three commands with three different verdicts, then a refused write ---
  {
    ...
    else if (!/^entrust: pid=\d+ identity=/.test(err.split("\n")[0] ?? ""))
...
async function main() {
  ...
  const liveFailed = process.env.ENTRUST_LIVE_TURN === "1" ? await liveTurns()
    : (console.log("\nlive turn: NOT RUN — set ENTRUST_LIVE_TURN=1 to spend one turn on the real server"), 0);
