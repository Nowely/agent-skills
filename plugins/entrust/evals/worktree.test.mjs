  ...
  async () => {
    ...
    const p = spawn(process.execPath,
      ...
      { env: { ...process.env, PATH: `${shimDir}:${process.env.PATH}`, FAKE_SCENARIO: "happy",
               ENTRUST_STATE_DIR: STATE_DIR }, stdio: ["ignore", "pipe", "pipe"] });
    let err = "";
...
test("the reconciler gives a crashed agent's commits a ref before it removes the tree that held them",
  ...
  async () => {
    ...
    for (const [name, planted] of [["codex-crash-based", withBase], ["codex-crash-legacy", noBase]]) {
      ...
      const log = spawnSync("git", ["-C", repo, "log", "--format=%s", `refs/entrust/${name}`], { encoding: "utf8" });
      if (log.status !== 0 || !log.stdout.includes(`crashed-${name}`))
        return `${name}: the commits were stranded — refs/entrust/${name} does not carry them ` +
...
test("a ledger that cannot be written stops the run before `git worktree add`",
  ...
  async () => {
    ...
    const { code, err } = await run(null, { args: ["--worktree", repo], env: { ENTRUST_STATE_DIR: state } });
