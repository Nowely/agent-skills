...
//   ENTRUST_LIVE_ORCHESTRATE=1 node evals/orchestrate-live.test.mjs --only 1,3
//
...
// prompt survives as a probe behind ENTRUST_LIVE_ORCHESTRATE_DELEGATE=1. Whether the agent
...
const DELEGATE_PROBE = process.env.ENTRUST_LIVE_ORCHESTRATE_DELEGATE === "1";
const skipped = [];
...
// then hold unchanged.
const SLUG_TASK =
  "/entrust:orchestrate TASK: add a slug(title) helper to lib/slug.mjs that lowercases, trims and "
...
const DESIGN_TASK =
  "/entrust:orchestrate TASK: compare two ways to give this repository's driver a --dry-run flag "
...
if (process.env.ENTRUST_LIVE_ORCHESTRATE !== "1") {
  console.log("live orchestrate gate: NOT RUN — set ENTRUST_LIVE_ORCHESTRATE=1 to spend real sessions on the orchestrate mode");
  process.exit(0);
}
