...
Do not publish from an unclean tree. Releases use annotated `entrust@X.Y.Z` tags and matching
GitHub release notes; never move or recreate a published tag. The repository is a marketplace and one
tag namespace serves every plugin in it, so the plugin's name is part of the tag. The series starts at
`entrust@0.16.0`; the seventeen `codex-delegate@*` tags before it are the same plugin under its previous name.
...
1. Choose the version and set it everywhere `evals/package.test.mjs` compares it:
   ...
   intended `entrust@X.Y.Z` tag match: the package eval compares them once such a tag is on `HEAD`, and
...
5. Run the local live fidelity gate separately:
   ...
   ```bash
   ENTRUST_LIVE_TURN=1 node evals/fidelity.test.mjs --require-live
   ```

   Verify the authenticated Codex build, inspect every fixture/live difference, and after a CLI change
   confirm the dated parity figures still hold as an order of magnitude. `ENTRUST_LIVE_TURN=1`
...
6. Run the live orchestrate gate. It is not in CI: it spends real sessions and a Codex turn.
   ...
   ```bash
   ENTRUST_LIVE_ORCHESTRATE=1 node evals/orchestrate-live.test.mjs
   ```

   Add `ENTRUST_LIVE_ORCHESTRATE_DELEGATE=1` after a codex upgrade: it spends a second Codex turn
...
8. Create one annotated tag form only:
   ...
   ```bash
   git tag -a entrust@X.Y.Z -m 'entrust X.Y.Z'
   ```
