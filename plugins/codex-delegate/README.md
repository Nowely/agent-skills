...
This route exposes the skill as `codex-delegate:codex`, the orchestrator mode as
...
`codex-delegate:codex`, `/codex-delegate:orchestrate`, `/codex-delegate:cleanup` and
`codex-delegate:codex-agent`. Agents are read when Claude Code starts, so a link made during a session is
